...
{
    VkVertexShader::VkVertexShader(const Ref<VkGraphicsDevice>& device, const std::vector<Ref<ShaderModule>>& shaderModules)
    {
        HL_EXCEPTION(shaderModules.size() <= 0, "At least 1 module must be present in VertexShader!");
        HL_EXCEPTION(Utils::HasDuplicateShaderModules(shaderModules), "You may not use the same ShaderModule multiple times!");
     
        m_key = ShaderKey(ShaderModule::Stage::Vertex, shaderModules);
        m_device = device;
        ...
        std::filesystem::path spirv = workingDir / "Cache" / "Shaders" / "Compiled" / ( hashString + "_vs" + ".spv");
        std::filesystem::path nameMap = workingDir / "Cache" / "Shaders" / "Compiled" / ( hashString + "_vs" + ".map");
        ...
        HL_EXCEPTION(mainModule == shaderModules.end(), "Main module is missing from VertexShader!");
        HL_EXCEPTION(!(*mainModule)->GetInputLayouts().has_value(), "Missing InputLayouts from Main module in VertexShader!");
        
        m_inputLayouts = (*mainModule)->GetInputLayouts().value();
        
        if (std::filesystem::exists(spirv) && std::filesystem::exists(nameMap))
        {
            auto code = Utils::readFile(spirv);
            Utils::createShaderModule(device, code, m_vertexShaderModule);
        ...
        if (save_hlsl)
        {
            std::filesystem::path icp = workingDir / "Cache" / "Shaders" / "Intermediate" / ( "generated_" + hashString + "_vs" + ".hlsl");
        ...
        auto result = CompileHLSLtoSPIRV(shaderCode, Shader::Stage::Vertex);
        
        HL_EXCEPTION(!result.has_value(), "Error compiling shader:\n" + result.error());

        HL_EXCEPTION(!ValidateInputLayoutsShader(result.value(), m_inputLayouts), "Error validating shader! Missmatch in InputLayouts!");
        ...
        Utils::createShaderModule(device, result.value(), m_vertexShaderModule);
    }
    VkVertexShader::VkVertexShader(const Ref<VkGraphicsDevice>& device, const std::filesystem::path &path, InputLayouts inputLayouts)
    {
        m_device = device;
        m_key = ShaderKey(ShaderModule::Stage::Vertex, {});

        // m_bindingLayouts = bindingLayouts;
        Utils::createShaderModule(device, path, m_vertexShaderModule, shaderc_shader_kind::shaderc_vertex_shader);
    }
}