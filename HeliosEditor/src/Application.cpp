...
namespace Helios
{
	HeliosEditor::HeliosEditor(const Specifications &specs) : Application(specs)
	{
		...
		GetWindow()->SetTitle(GetWindow()->GetTitle() + " | Project: " + Project::GetProjectSettings().name);
		...
		try
		{
			constexpr static int to_load_count = 24;

			initWindow->SetB("Loading icon. 1 out of " + std::to_string(to_load_count), to_load_count);
			ICON_FOLDER_EMPTY = Texture2D::Create("Resources/Icons/folder_empty.png");
			initWindow->SetB("Loading icon. 2 out of " + std::to_string(to_load_count), to_load_count);
			ICON_FILE_UNKNOWN = Texture2D::Create("Resources/Icons/unknown_file.png");
			initWindow->SetB("Loading icon. 3 out of " + std::to_string(to_load_count), to_load_count);
			ICON_FILE_TXT = Texture2D::Create("Resources/Icons/text_file.png");
			initWindow->SetB("Loading icon. 4 out of " + std::to_string(to_load_count), to_load_count);
			ICON_FILE_C = Texture2D::Create("Resources/Icons/c_file.png");
			initWindow->SetB("Loading icon. 5 out of " + std::to_string(to_load_count), to_load_count);
			ICON_FILE_H = Texture2D::Create("Resources/Icons/h_file.png");
			initWindow->SetB("Loading icon. 6 out of " + std::to_string(to_load_count), to_load_count);
			ICON_FILE_CPP = Texture2D::Create("Resources/Icons/cpp_file.png");
			initWindow->SetB("Loading icon. 7 out of " + std::to_string(to_load_count),  to_load_count);
			ICON_FILE_HPP = Texture2D::Create("Resources/Icons/hpp_file.png");
			initWindow->SetB("Loading icon. 8 out of " + std::to_string(to_load_count), to_load_count);
			ICON_FILE_RUST = Texture2D::Create("Resources/Icons/rust_file.png");
			initWindow->SetB("Loading icon. 9 out of " + std::to_string(to_load_count), to_load_count);
			ICON_FILE_FONT = Texture2D::Create("Resources/Icons/ttf_file.png");
			initWindow->SetB("Loading icon. 10 out of " + std::to_string(to_load_count), to_load_count);
			ICON_FILE_SCENE = Texture2D::Create("Resources/Icons/scene_file.png");
			initWindow->SetB("Loading icon. 11 out of " + std::to_string(to_load_count),  to_load_count);
			ICON_FILE_IMAGE = Texture2D::Create("Resources/Icons/image_file.png");
			initWindow->SetB("Loading icon. 12 out of " + std::to_string(to_load_count),  to_load_count);
			ICON_FILE_OBJECT = Texture2D::Create("Resources/Icons/object_file.png");
			initWindow->SetB("Loading icon. 13 out of " + std::to_string(to_load_count),  to_load_count);
			ICON_PLAY_WHITE = Texture2D::Create("Resources/Icons/play_white.png");
			initWindow->SetB("Loading icon. 14 out of " + std::to_string(to_load_count),  to_load_count);
			ICON_PAUSE_WHITE = Texture2D::Create("Resources/Icons/pause_white.png");
			initWindow->SetB("Loading icon. 15 out of " + std::to_string(to_load_count), to_load_count);
			ICON_STOP_WHITE = Texture2D::Create("Resources/Icons/stop_white.png");
			initWindow->SetB("Loading icon. 16 out of " + std::to_string(to_load_count),  to_load_count);
			ICON_FOLDER = Texture2D::Create("Resources/Icons/folder.png");

			initWindow->SetB("Loading icon. 17 out of " + std::to_string(to_load_count), to_load_count);
			ICON_UI_GROUP_TILES_WHITE = Texture2D::Create("Resources/ui_explorer_group_tiles_white.png");
			initWindow->SetB("Loading icon. 18 out of " + std::to_string(to_load_count), to_load_count);
			ICON_UI_GROUP_LIST_WHITE = Texture2D::Create("Resources/ui_explorer_group_list_white.png");
			initWindow->SetB("Loading icon. 19 out of " + std::to_string(to_load_count),  to_load_count);
			ICON_UI_GROUP_ICONS_WHITE = Texture2D::Create("Resources/ui_explorer_group_icons_white.png");

			initWindow->SetB("Loading icon. 20 out of " + std::to_string(to_load_count),  to_load_count);
			ICON_UI_GROUP_TILES_BLACK = Texture2D::Create("Resources/ui_explorer_group_tiles_black.png");
			initWindow->SetB("Loading icon. 21 out of " + std::to_string(to_load_count),  to_load_count);
			ICON_UI_GROUP_LIST_WHITE = Texture2D::Create("Resources/ui_explorer_group_list_black.png");
			initWindow->SetB("Loading icon. 22 out of " + std::to_string(to_load_count), to_load_count);
			ICON_UI_GROUP_ICONS_WHITE = Texture2D::Create("Resources/ui_explorer_group_icons_black.png");

			initWindow->SetB("Loading icon. 23 out of " + std::to_string(to_load_count), to_load_count);
			ICON_UI_REFRESH_WHITE = Texture2D::Create("Resources/ui_refresh_white.png");
			initWindow->SetB("Loading icon. 24 out of " + std::to_string(to_load_count), to_load_count);
			ICON_UI_REFRESH_BLACK = Texture2D::Create("Resources/ui_refresh_black.png");
		}
		catch (HeliosException e)
		{
			MessageBoxA(nullptr, "Error loading icons!", e.what().c_str(), MB_ICONERROR);
			exit(-100);
		}
		...
		gameFrame = Framebuffer::Create(300, 300, {Framebuffer::Format::R8G8B8A8_UNORM,
		...
		auto ent = SceneRegistry::GetActiveScenes()[0]->CreateEntity(Vector3{0, 0, 0});
		...
		mrc.material = Material::Create(Material::Filter::MinMagMipPoint, Material::Type::Clamp);
		auto ent2 = SceneRegistry::GetActiveScenes()[0]->CreateEntity(Vector3{0, 0, 0});
		...
		RustScriptsManager rsm = RustScriptsManager(Project::GetProjectPath() / "BuildSystems/Rust/target/release/helios_rust.dll");
	}
	...
	void HeliosEditor::OnUpdate()
	{
		...
		//{
		//	DepricatedHelios::ShowMessage("INFO!", "Toggled GUI", MB_ICONINFORMATION, false);
		//	//hideGui = !hideGui;
		// }
		...
		if (InputManager::IsKeyPressed(HL_KEY_CONTROL) && InputManager::IsKeyPressed(HL_KEY_F11))
		{
			MessageBoxA(nullptr, "Toggled Per Frame Debug", "INFO!", MB_ICONINFORMATION);
		...
		if (m_perFrameDebug)
		{
			while (!(InputManager::IsKeyPressed(HL_KEY_CONTROL) && InputManager::IsKeyPressed(HL_KEY_N)))
			{
			...
			MessageBoxA(nullptr, "Next Frame", "INFO!", MB_ICONINFORMATION);
		}
	}
	...
	void HeliosEditor::OnRender()
	{
		if (m_barebones)
		{
			...
			SceneRegistry::OnEditorRender(tmp, camera);
		}
		else
		{
