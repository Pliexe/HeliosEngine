...
namespace Helios
{
    ...
    struct TVectorOps
    {
        ...
        static inline Derived MoveTowards(const Derived& lhs, const Derived& rhs, float maxDistanceDelta) requires IsVector { return lhs + (rhs - lhs).normalize() * maxDistanceDelta; }
        ...
        static inline Derived ProjectOnPlane(const Derived& pointOnPlane, const Derived& planeNormal, const Derived& point) requires IsVector { return point - Project(point - pointOnPlane, planeNormal); }

        // Ik this is not ops but who can stop me?
        inline std::string to_string() requires IsVector 
        {
