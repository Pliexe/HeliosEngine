...
namespace Helios
{
	void GameViewPanel::OnUpdate()
	{
		...
		if (ImGui::Begin("Game", 0, m_flags))
		{
			...
			// 	{
			// 		if (viewportSize.x > 0 && viewportSize.y > 0) static_cast<HeliosEditor&>(HeliosEditor::GetInstance()).gameFrame->Resize(viewportSize.x, viewportSize.y);
			// 	}
			// 	// if (showDepthBuff)
