...
#pragma region Initilization Shaders
		...
		s_Data.standard_gizmos_shader = DepricatedShader::Create("StandardGizmos", "CompiledShaders/StandardGizmosVertexShader", "CompiledShaders/StandardGizmosPixelShader", s_Data.standard_gizmos_input_layouts, DepthFunc::Always, Topology::TriangleList);

		s_Data.quad_gizmos_shader = DepricatedShader::Create("QuadGizmo", "CompiledShaders/QuadGizmoVertexShader", "CompiledShaders/QuadGizmoPixelShader", s_Data.quad_gizmos_input_layouts, DepthFunc::Always, Topology::TriangleList);

		s_Data.line_gizmos_shader = DepricatedShader::Create("Line", "CompiledShaders/LineVertexShader", "CompiledShaders/LinePixelShader", "CompiledShaders/LineGeometryShader", s_Data.line_gizmos_input_layouts, DepthFunc::Always, Topology::LineList);

		s_Data.dynamic_gizmos_shader = DepricatedShader::Create("DynamicGizmos", "CompiledShaders/DynamicGizmosVertexShader", "CompiledShaders/DynamicGizmosPixelShader", s_Data.dynamic_gizmos_input_layouts, DepthFunc::Always, Topology::TriangleList);
...
#pragma endregion
    ...
    void GizmosRenderer::Shutdown()
    {
		s_Data.quads.vertexArray.reset();
		s_Data.quad_gizmos_shader.reset();
		s_Data.standard_gizmos_shader.reset();
		s_Data.tool_gizmos_shader.reset();
		s_Data.line_gizmos_shader.reset();
		s_Data.dynamic_gizmos_shader.reset();
		s_Data.m_GizmosInstanceData.m_InstanceBuffer.reset();
		s_Data.m_lines.m_vertexBuffer.reset();
		s_Data.m_dynamic_gizmos.m_vertex_buffer.reset();
		s_Data.m_GizmosInstanceData.ArrowGizmo->m_VertexBuffer.reset();
		s_Data.m_GizmosInstanceData.ArrowGizmo->m_IndexBuffer.reset();
		s_Data.m_GizmosInstanceData.SphereGizmo->m_VertexBuffer.reset();
		s_Data.m_GizmosInstanceData.SphereGizmo->m_IndexBuffer.reset();
		s_Data.m_GizmosInstanceData.TorusGizmo->m_VertexBuffer.reset();
		s_Data.m_GizmosInstanceData.TorusGizmo->m_IndexBuffer.reset();
		s_Data.transformBuffer.reset();
...
#define MAX_CIRCLE_SEGMENTS 64
	...
	void GizmosRenderer::DrawMeshVertices(EditorCamera camera, TransformComponent& transform, std::vector<MeshVertex>& vertices)
	{
		//static Matrix4x4 scale = Matrix4x4::Scale({ 0.1f, 0.1f, 1.0f });
		Matrix4x4 scale = Matrix4x4::Scale((transform.Position - camera.GetPosition()).length() * 0.05f);
		...
		HL_PROFILE_BEGIN("GizmosRenderer - (DrawMeshVertices) Draw Vertices");
		...
		for(uint32_t i = 0, n = vertices.size(); i < n; i++)
		{
			if (s_Data.quads.quadInstanceIndex >= GizmosData::MaxQuadInstances)
				Flush();
			...
			s_Data.quads.quadInstances[s_Data.quads.quadInstanceIndex] = {
				Matrix4x4::Transpose(
					worldMatrix *
					// Matrix4x4::Scale(std::max((vertices[i].position - camera.GetPosition()).length() * 0.5f, 0.5f)) *
					vertexMatrix *
					Matrix4x4::TranslationRow(vertices[i].position * transform.Scale)
				),
				Color::Yellow, (int32_t)i
			};
			// s_Data.quads.quadInstances[s_Data.quads.quadInstanceIndex] = {
			// 	worldMatrix,
			// 	Color::Yellow, i
	...
	void GizmosRenderer::DrawTool(Transform transform, EditorCamera camera, ToolType type, Tool operation)
	{
		...
		Matrix4x4 transform_matrix = (
			Matrix4x4::TranslationColumn(transform_component.Position) *
			Matrix4x4::RotationColumn(transform_component.Rotation) *
			Matrix4x4::Scale((transform.GetWorldPositionCache() - camera.GetPosition()).length() * 0.3f)
		);
	...
	void GizmosRenderer::DrawWireCube(const TransformComponent& transform, const Vector3& scale)
	{
		//Vector2 p1 = transform.Scale;
		//Vector2 p2 = { p1.x, -p1.y };
		//Vector2 p3 = { -p1.x, -p1.y };
		//Vector2 p4 = { -p1.x, p1.y } * scale;
	}
	...
	void GizmosRenderer::DrawWireQuad(const TransformComponent& transform, const Vector3& scale, const float line_width)
	{
        float rotation = Quaternion::ToEulerRads(transform.Rotation).z;
        const Vector2 all_scale = transform.Scale * 0.5f * scale;
        Vector2 p1 = transform.Position + all_scale;
        Vector2 p2 = transform.Position + Vector2 {  all_scale.x, -all_scale.y };
        Vector2 p3 = transform.Position + Vector2 { -all_scale.x, -all_scale.y };
        Vector2 p4 = transform.Position + Vector2 { -all_scale.x,  all_scale.y };
        ...
        DrawLine(p1, p2, line_width, Color::Green, -1, LineMode::Solid);
        DrawLine(p2, p3, line_width, Color::Green, -1, LineMode::Solid);
        DrawLine(p3, p4, line_width, Color::Green, -1, LineMode::Solid);
        DrawLine(p4, p1, line_width, Color::Green, -1, LineMode::Solid);
	}
