...
namespace Helios {
	...
	void Scene::RenderScene(Ref<Image>& colorBuffer, EditorCamera& camera)
	{
	...
	void Scene::PerformCleanupAndSync()
	{
		...
		if (Physics2D::GetImplementationType() != Physics2D::ImplementationType::None)
		{
	...
	void Scene::RenderScene(Ref<Image>& colorBuffer, TransformComponent world_camera, Matrix4x4 projection)
	{
		...
		// Renderer::BeginScene(colorBuffer, projection, world_camera.Position, {1.0f, 1.0f, 1.0f, 0.2f}, m_components);
		...
		// 	m_components.view<TransformComponent, RelationshipComponent, MeshRendererComponent>(entt::exclude<DisabledObjectComponent>).each([&](auto entity, TransformComponent& transform, RelationshipComponent& relationship, MeshRendererComponent& meshRenderer)
		// 	{
	...
	void Scene::RenderGizmos(Matrix4x4 projection)
	{
		...
		// for (const auto view = m_components.view<TransformComponent, BoxCollider2D>(entt::exclude<DisabledObjectComponent>); auto& entity : view)
		// {
		// 	auto [transform, boxCollider] = view.get<TransformComponent, BoxCollider2D>(entity);
		// 	auto relationship = m_components.get<RelationshipComponent>(entity);
		// 	const Vector3 vec = boxCollider.size;
		// 	TransformComponent wt = transform.GetWorldTransform(relationship, m_components);
		// 	wt.Position += boxCollider.offset;
		// 	GizmosRenderer::DrawWireQuad(wt, vec);
		// }
	...
	void Scene::Serialize(const std::string& filepath, WeakRef<Scene>& scene)
	{
		...
		scene.lock()->m_components.view<entt::entity>().each([&](entt::entity entity) {
