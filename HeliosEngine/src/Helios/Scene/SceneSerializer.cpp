...
{
    HL_ASSERT(entity.HasComponent<UUIDComponent>(),
              "Entity has no uuid component!");
    HL_ASSERT(entity.HasComponent<InfoComponent>(),
              "Entity has no tag component!");

    auto container = entity.GetContainer().lock();

    if (!container)
        HL_ASSERT(false, "Entity has no container!");
    ...
    if (entity.HasComponent<CameraComponent>())
    {
        ...
        out << YAML::Key << "IsOrthographic" << YAML::Value
            << camera.ortographic;
    ...
    if (entity.HasComponent<RelationshipComponent>())
    {
        ...
        out << YAML::Key << "Parent" << YAML::Value
            << container->GetEntity(relationship.parent_handle);
        out << YAML::Key << "Top" << YAML::Value
            << container->GetEntity(relationship.top_handle);

        out << YAML::Key << "Siblings" << YAML::Value << YAML::BeginMap;

        out << YAML::Key << "First" << YAML::Value
            << container->GetEntity(relationship.first_child);
        out << YAML::Key << "Next" << YAML::Value
            << container->GetEntity(relationship.next_sibling);
        out << YAML::Key << "Previous" << YAML::Value
            << container->GetEntity(relationship.prev_sibling);
    ...
    if (entity.HasComponent<MeshRendererComponent>())
    {
        ...
        // if (mesh.material->texture != nullptr) out << YAML::Key << "Material"
        // << YAML::Value << mesh.material->texture-> out << YAML::Key <<
        // "Material" << YAML::Value << mesh.material;
        out << YAML::EndMap;
    }
    ...
    if (entity.HasComponent<Rigidbody2D>())
    {
        ...
        out << YAML::Key << "IsKinematic" << YAML::Value
...
void SceneSerializer::SerializeEditor(const std::string& filepath) const
{
    ...
    m_scene->m_components.view<entt::entity>().each(
...
void DeserializeEntity(YAML::Node entityNode, Scene* scene)
{
    ...
    Helios::UUID uuid = entityNode["Entity"].as<Helios::UUID>();
    ...
    if (entityNode["Transform"])
    {
        ...
        transformComponent.Rotation =
            transformNode["Rotation"].as<Quaternion>();
    ...
    {
        auto& lightComponent =
            entity.AddScopedComponent<DirectionalLightComponent>();
    ...
    if (entityNode["SpriteRenderer"])
    {
        auto& spriteComponent =
            entity.AddScopedComponent<SpriteRendererComponent>();
    ...
    {
        auto& meshComponent =
            entity.AddScopedComponent<MeshRendererComponent>();
        auto meshNode = entityNode["MeshRenderer"];
        meshComponent.material =
            MaterialInstance::Create(MaterialInstance::Filter::MinMagMipPoint,
                                     MaterialInstance::Type::Clamp);
        meshComponent.material->AlbedoColor = meshNode["Color"].as<Color>();
        if (meshNode["Texture"])
            meshComponent.material->texture =
                ResourceRegistry::GetResource<Texture2D>(
                    meshNode["Texture"].as<Helios::UUID>());
        meshComponent.mesh = ResourceRegistry::GetResource<Mesh>(
            meshNode["Mesh"].as<Helios::UUID>());
    }
    if (entityNode["Rigidbody2D"])
    {
        ...
        rigidbodyComponent.isKinematic =
            rigidbodyNode["IsKinematic"].as<bool>();
...
void CheckRelationship(YAML::Node entityNode, Scene* scene)
{
    ...
    {
        Entity entity =
            scene->GetEntity(entityNode["Entity"].as<Helios::UUID>());

        auto relationshipNode = entityNode["Relationship"];

        if (relationshipNode["Parent"] &&
            relationshipNode["Parent"].as<std::string>() !=
                "0") // 0 means no uuid (null)
        {
            Helios::UUID parentUUID =
                relationshipNode["Parent"].as<Helios::UUID>();
            Entity parentEntity = scene->GetEntity(parentUUID);
            entity.GetComponent<RelationshipComponent>().parent_handle =
                parentEntity.GetHandle();
        }

        if (relationshipNode["Top"] &&
            relationshipNode["Top"].as<std::string>() != "0")
        {
            Helios::UUID topUUID = relationshipNode["Top"].as<Helios::UUID>();
            Entity topEntity = scene->GetEntity(topUUID);
            entity.GetComponent<RelationshipComponent>().top_handle =
                topEntity.GetHandle();
        }

        if (relationshipNode["Siblings"] &&
            relationshipNode["Siblings"].IsMap())
        {
            auto siblings = relationshipNode["Siblings"];
            if (siblings["First"] && siblings["First"].as<std::string>() != "0")
            {
                Helios::UUID firstUUID = siblings["First"].as<Helios::UUID>();
                Entity firstEntity = scene->GetEntity(firstUUID);
                entity.GetComponent<RelationshipComponent>().first_child =
                    firstEntity.GetHandle();
            }
            if (siblings["Next"] && siblings["Next"].as<std::string>() != "0")
            {
                Helios::UUID nextUUID = siblings["Next"].as<Helios::UUID>();
                Entity nextEntity = scene->GetEntity(nextUUID);
                entity.GetComponent<RelationshipComponent>().next_sibling =
                    nextEntity.GetHandle();
            }
            if (siblings["Previous"] &&
                siblings["Previous"].as<std::string>() != "0")
            {
                Helios::UUID previousUUID =
                    siblings["Previous"].as<Helios::UUID>();
                Entity previousEntity = scene->GetEntity(previousUUID);
                entity.GetComponent<RelationshipComponent>().prev_sibling =
                    previousEntity.GetHandle();
            }
        }
    }
}
